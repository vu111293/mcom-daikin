...
//    ));
  ...
  Future<List<NewsModel>> getNotifications() async {
    final result = await graphQLAPI.getNotifications();
    if (result.data['notifications']['data'] == null) return [];
    return ((result.data as dynamic)['notifications']['data'] as List).map((item) => NewsModel.fromJson(item)).toList();
  }

  Future makeRead(String id) async {
    final result = await graphQLAPI.makeRead(id);
    return result;
  }
}
